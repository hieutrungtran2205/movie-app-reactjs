...
function DetailMovie({ id, type, getCastId }) {
  ...
  //     const url = `https://api.themoviedb.org/3/${type}/${id}/similar?api_key=cb1cb02271e379c911c6d9401d1eb7f8&language=vi`;
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      const url = `https://api.themoviedb.org/3/${type}/${id}?api_key=cb1cb02271e379c911c6d9401d1eb7f8&language=vi`;
      const response = await fetch(url);
      const data = await response.json();
      setMovieDetail(data);
  ...
  useEffect(() => {
    const getTrailer = async () => {
      const url = `https://api.themoviedb.org/3/${type}/${id}/videos?api_key=cb1cb02271e379c911c6d9401d1eb7f8`;
      const response = await fetch(url);
      const data = await response.json();
  ...
  const countries = productCountry.map((country) => {
  ...
  return (
    <>
            ...
            <div className="tt-detail">
              ...
              <div className="main-column">
                ...
                <div className="meta">
                  ...
                  <span> {movieDetail.vote_count} lượt xem</span>
                </div>
                ...
                <div className="meta">
                  ...
                  <span className="imb-rating">{movieDetail.vote_average}</span>
                </div>
                ...
                <div className="cast">
                  <h3>Diễn viên</h3>
                  <Cast id={id} type={type} getCastId={getCastId} />
                </div>
      ...
      {/* <h5>Các phim cùng thể loại</h5>
      <Poster type={type} getId={id} filmData={similar} number={8} /> */}
    </>
  );
}
